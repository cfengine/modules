...
Patterns are unanchored (substring match), so =UEFI dbx= matches any
device whose name contains "UEFI dbx". Use =^…$= to anchor if you need
an exact match. Use =.*= to match all devices.
