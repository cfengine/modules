...
from cfengine_module_library import ValidationError  # noqa: E402


@pytest.fixture
def module():
    return AppStreamsPromiseTypeModule()


def test_validation_valid_attributes(module):
    """Test validation of valid module attributes"""
    module.validate_promise("nodejs", {"state": "enabled", "stream": "12"}, {})


def test_validation_invalid_module_name(module):
    """Test validation of invalid module name"""
    with pytest.raises(ValidationError) as excinfo:
        module.validate_promise("nodejs; echo hi", {"state": "enabled"}, {})
    assert "Invalid module name" in str(excinfo.value)


@pytest.mark.parametrize(
    "name", ["nodejs", "python3.6", "python36", "postgresql", "maven", "httpd"]
)
...
@pytest.mark.parametrize(
    "name", ["nodejs;echo", "python36&&", "postgresql|", "maven>", "httpd<"]
)
...
@pytest.mark.parametrize(
    "stream", ["12", "14", "3.6", "1.14", "latest", "stable", "default"]
)
...
@pytest.mark.parametrize("stream", ["12;echo", "14&&", "3.6|", "latest>", "stable<"])
...
@pytest.mark.parametrize(
    "state", ["enabled", "disabled", "installed", "removed", "default", "reset"]
)
...
@pytest.mark.parametrize("profile", ["common", "minimal", "server", "default", "1.0"])
def test_profile_name_validation_valid(module, profile):
    """Test profile name validation with valid names"""
    module._validate_profile_name(profile)


@pytest.mark.parametrize("profile", ["common;echo", "minimal&&", "server|", "default>"])
def test_profile_name_validation_invalid(module, profile):
    """Test profile name validation with invalid names"""
    with pytest.raises(ValidationError):
        module._validate_profile_name(profile)
