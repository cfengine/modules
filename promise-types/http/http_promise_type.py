...
class HTTPPromiseModule(PromiseModule):
    ...
    def validate_promise(self, promiser, attributes, metadata):
        ...
        if "method" in attributes:
            ...
            if method not in _SUPPORTED_METHODS:
                raise ValidationError(
                    "'method' must be one of %s" % ", ".join(_SUPPORTED_METHODS)
                )
        ...
        if "headers" in attributes:
            ...
            if headers_type is str:
                ...
                raise ValidationError(
                    "'headers' must be a string, an slist or a data container"
        ...
        if "payload" in attributes:
            ...
            if type(payload) not in (str, dict):
                raise ValidationError(
                    "'payload' must be a string or a data container value"
                )
    ...
    def target_fh(self, file_info):
        if file_info.target:
            ...
            with open(temp_file, "wb") as fh:
                yield fh
            if not os.path.isfile(file_info.target) or not filecmp.cmp(
                temp_file, file_info.target
            ):
    ...
    def evaluate_promise(self, promiser, attributes, metadata):
        ...
        canonical_promiser = promiser.translate(
        ...
        if payload:
            if isinstance(payload, dict):
                try:
                    ...
                    self.log_error(
                        "Failed to convert 'payload' to text representation for request '%s'"
                        % url
                    )
                    return (
                        Result.NOT_KEPT,
                        [
                            ...
                            "%s_%s_payload_conversion_failed"
                            % (canonical_promiser, method),
                        ],
                    )
                ...
                try:
                    ...
                    self.log_error(
                        "Failed to open payload file '%s' for request '%s': %s"
                        % (path, url, e)
                    )
                    return (
                        Result.NOT_KEPT,
        ...
        try:
            with urllib.request.urlopen(request, context=SSL_context) as url_req:
                if not (200 <= url_req.status < 300):
                    self.log_error(
                        "Request for '%s' failed with code %d" % (url, url_req.status)
                    )
                    return (
                        Result.NOT_KEPT,
                        ["%s_%s_request_failed" % (canonical_promiser, method)],
            ...
            self.log_error(
                "Failed to store '%s' response to '%s': %s" % (url, target, e)
            )
            return (
                Result.NOT_KEPT,
                [
        ...
        if target:
            if result == Result.REPAIRED:
                self.log_info(
                    "Saved request response from '%s' to '%s'" % (url, target)
                )
            else:
                self.log_info(
                    "No changes in request response from '%s' to '%s'" % (url, target)
                )
        else:
            self.log_info(
                "Successfully executed%s request to '%s'"
                % ((" " + method if method else ""), url)
            )
